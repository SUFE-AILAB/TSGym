...
class Meta():
    ...
    def get_meta_feature(self,meta_feature_path,mode,datasets):
        '''
        函数用于读取meta feature
        INPUT:
        meta_feature_path: str 保存meta feature的文件目录
        mode: str 读取meta feature的类型
        - old: 旧版本的meta feature
        - whole: 序列整体meta feature
        - subseries: 分段子序列meta feature
        - delta: 描述序列漂移feature
        - whole_delta: 整体+序列漂移feature
        - whole_subseries: 整体+子序列
        - whole_subseries_delta: 整体+子序列+分布漂移
        - datasets: 数据集列表，和TSGYM对齐
        RETURN:
        meta_features_dict: dict 数据集名称:meta_feature numpy array
        '''
        ...
        if mode == 'old':
            ...
            mode = 'whole' # 旧版本的meta feature只有全部feature
        if 'whole' in mode:
            meta_features_whole = {dataset: np.load(f'{meta_feature_path}/meta_feature_{name_dict[dataset]}_train_{pred_dict[dataset]}.npz',
                                                    allow_pickle=True)['meta_feature'].flatten() for dataset in datasets}
        if 'subseries' in mode:
            meta_features_subseries = {dataset: np.load(f'{meta_feature_path}/meta_feature_{name_dict[dataset]}_train_{pred_dict[dataset]}.npz',
                                                    allow_pickle=True)['subseries_meta_feature'].flatten() for dataset in datasets}
        if 'delta' in mode:
            meta_features_delta = {}
            for dataset in datasets:
                ...
                mean, std = np.mean(delta_series,axis=0), np.std(delta_series,axis=0)
                min_val,q25,median, = np.min(delta_series,axis=0),np.percentile(delta_series, 25,axis=0),np.median(delta_series,axis=0)
                q75,max_val= np.percentile(delta_series, 75,axis=0),np.max(delta_series,axis=0)
                range_val = max_val - min_val
                iqr = q75 - q25
                combined_features = np.stack([mean, std, min_val, q25, median, q75, max_val, range_val, iqr])#(9,)
                meta_features_delta[dataset] = combined_features
        if mode == 'whole':
            meta_features = meta_features_whole
        elif mode == 'subseries':
            meta_features = meta_features_subseries
        elif mode == 'delta':
            meta_features = meta_features_delta
        elif mode == 'whole_delta':
            meta_features = {key: np.concatenate((meta_features_whole[key], meta_features_delta[key])) for key in meta_features_whole}
        elif mode == 'subseries_delta':
            meta_features = {key: np.concatenate((meta_features_subseries[key], meta_features_delta[key])) for key in meta_features_subseries}
        elif mode == 'whole_subseries':
            meta_features = {key: np.concatenate((meta_features_whole[key], meta_features_subseries[key])) for key in meta_features_whole}
        elif mode == 'whole_subseries_delta':
            meta_features = {key: np.concatenate((meta_features_whole[key], meta_features_subseries[key], meta_features_delta[key])) for key in meta_features_whole}
        else:
    ...
    def components_processing(self, task_name, datasets, test_dataset,
                              ...
                              meta_feature_path='./get_meta_feature/meta_features_new',
                              meta_feature_mode='whole',
        ...
        # load meta features
        self.meta_features = self.get_meta_feature(meta_feature_path,meta_feature_mode,datasets)
    ...
    def meta_fit(self, best_metric=999, best_epoch=0, es_count=0, es_tol=5, es_stopped=False):
        ...
        np.savez_compressed(f'./meta_cc/{self.save_path_dir}/{self.test_dataset}-component_balance_{self.arg_component_balance}-add_new_dataset_{self.arg_add_new_dataset}-add_transformer_{self.arg_add_transformer}_{self.pred_len_1}_{self.pred_len_2}.npz',
...
os.makedirs('meta_cc', exist_ok=True)
os.makedirs('meta_cc/logfiles', exist_ok=True)


logging.basicConfig(filename=f'meta_cc/logfiles/meta.log', filemode='a', format='%(asctime)s - %(message)s', level=logging.INFO)
...
datasets = ['ETTh1', 'ETTh2', 'Exchange', 'ili','ETTm1','ETTm2','ECL', 'traffic', 'weather']
utils=Utils()
utils.set_seed(42)
for meta_feature_mode in ['whole_subseries_delta','old','whole','subseries','whole_delta']:
    for pred_len_1,pred_len_2 in [(96,24),(192,36),(336,48),(720,60)]:
        save_path = f'result_{meta_feature_mode}'
        os.makedirs(f'meta_cc/{save_path}', exist_ok=True)
